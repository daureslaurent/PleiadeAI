...
/**
 * One `project → agent` routing row (`GITLAB_PLAN.md` §5): which agent a webhook about this project
 * wakes, when the event named nobody. `project` is the full path (`group/sub/project`).
 */
const GitLabProjectAgentSchema = new Schema(
  {
...
const SettingsSchema = new Schema(
  {
    ...
    /**
     * **GitLab** (`GITLAB_PLAN.md` §1) — one bot account the whole fleet works through.
     *
     * `gitlab_url` is the instance root (no trailing slash, no `/api/v4`: the client appends it).
     * The three `_enc` fields are AES-256-GCM (same helper as the isolation SSH keys), `select:
     * false` so an ordinary settings read never carries them, and `_enc$`-suffixed so `redact.ts`
     * scrubs them from API-key responses even if one did. `gitlab_group`, when set, confines every
     * listing, search and project lookup to that namespace's subtree — a guard re-checked per call,
     * not a filter applied to results.
     */
    gitlab_url: { type: String, default: '' },
    gitlab_token_enc: { type: String, default: '', select: false },
    gitlab_group: { type: String, default: '' },
    /** The bot account's own username — how a webhook recognises "assigned to us". */
    gitlab_bot_username: { type: String, default: '' },
    /** `X-Gitlab-Token` the webhook route demands. Generated by the settings page, shown once. */
    gitlab_webhook_secret_enc: { type: String, default: '', select: false },
    /** Who a webhook wakes when no `@name` and no per-project row matches. '' → nothing runs. */
    gitlab_default_agent_id: { type: String, default: '' },
    /** Per-project routing overrides, edited as rows on the settings page. */
    gitlab_project_agents: { type: [GitLabProjectAgentSchema], default: [] },
    /**
     * Both ship **off**, like `forum_auto_reply`: an inbound endpoint that spends inference the
     * moment it becomes reachable should be armed deliberately, not by being configured.
     */
     ...
     */
    gitlab_git_transport: { type: String, enum: ['https', 'ssh'], default: 'https' },
    gitlab_ssh_key_enc: { type: String, default: '', select: false },
