...
// ---- Internal git (Forgejo, GIT_SERVER_PLAN.md) ----------------------------------------------------
...
export interface GitStatus {
  ...
  version?: string;
  server_url?: string;
  /** Clone base URL per isolation network mode (null where that mode has no route). */
  agent_urls?: Record<'bridge' | 'host' | 'vpn', string | null>;
  reachable?: boolean;
  error?: string;
}
...
export interface GitAccess {
  fleet_readable: boolean;
  agents: (GitAgentRef & { permission: GitPermission; explicit: boolean })[];
}
...
const repoUrl = (repo: string) => `/git/repos/${encodeURIComponent(repo)}`;

export const gitApi = {
  status: () => api.get<GitStatus>('/git/status').then((r) => r.data),
  identities: () => api.get<GitIdentity[]>('/git/identities').then((r) => r.data),
  provision: (agentId: string) => api.post(`/git/identities/${agentId}/provision`).then((r) => r.data),
  rotate: (agentId: string) => api.post(`/git/identities/${agentId}/rotate`).then((r) => r.data),
  repos: () => api.get<GitRepo[]>('/git/repos').then((r) => r.data),
  repo: (repo: string) => api.get<GitRepo>(repoUrl(repo)).then((r) => r.data),
  createRepo: (body: { name: string; description?: string; fleet_readable?: boolean }) =>
    api.post<GitRepo>('/git/repos', body).then((r) => r.data),
  updateRepo: (repo: string, body: { description: string }) =>
    api.patch<GitRepo>(repoUrl(repo), body).then((r) => r.data),
  deleteRepo: (repo: string) => api.delete(repoUrl(repo)).then((r) => r.data),
  tree: (repo: string, path: string, ref?: string) =>
    api.get<GitTreeEntry[]>(`${repoUrl(repo)}/tree`, { params: { path, ref } }).then((r) => r.data),
  file: (repo: string, path: string, ref?: string) =>
    api.get<GitFile>(`${repoUrl(repo)}/raw`, { params: { path, ref } }).then((r) => r.data),
  commits: (repo: string, opts: { ref?: string; path?: string; page?: number } = {}) =>
    api
      .get<{ items: GitCommitSummary[]; hasMore: boolean }>(`${repoUrl(repo)}/commits`, { params: opts })
      .then((r) => r.data),
  commit: (repo: string, sha: string) =>
    api.get<GitCommitDetail>(`${repoUrl(repo)}/commits/${sha}`).then((r) => r.data),
  branches: (repo: string) => api.get<GitBranch[]>(`${repoUrl(repo)}/branches`).then((r) => r.data),
  access: (repo: string) => api.get<GitAccess>(`${repoUrl(repo)}/access`).then((r) => r.data),
  setAccess: (
    repo: string,
    body: { fleet_readable?: boolean; agent_id?: string; permission?: GitPermission },
  ) => api.put<GitAccess>(`${repoUrl(repo)}/access`, body).then((r) => r.data),
  activity: (opts: { agentId?: string; repo?: string; page?: number } = {}) =>
    api
      .get<{ items: GitActivity[]; hasMore: boolean }>('/git/activity', { params: opts })
      .then((r) => r.data),
};
