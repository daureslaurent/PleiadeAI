...
/**
 * Date buckets for an agent's conversation list, newest first. A long list reads as a history rather
 * than a wall of rows once "today" is visibly separate from "last August" — and the labels survive
 * paging, since a bucket simply gains rows as more are loaded.
 */
...
function bucketOf(iso: string): (typeof BUCKETS)[number] {
  ...
  // Calendar days, not elapsed hours: something from 23:00 last night is "Yesterday", not "Today".
...
/** Group a window of sessions into the non-empty buckets, preserving the newest-first order. */
function groupByDate(sessions: Session[]): { label: string; items: Session[] }[] {
  const groups = new Map<string, Session[]>();
  for (const sn of sessions) {
    const label = bucketOf(sn.updated_at);
    const items = groups.get(label);
    if (items) items.push(sn);
    else groups.set(label, [sn]);
  }
...
interface Props {
  ...
  /** The *loaded window* of each agent's sessions — not necessarily all of them. */
  sessionsByAgent: Record<string, Session[]>;
  /** How many sessions each agent has in total, so the pager can count what's still hidden. */
  sessionTotals: Record<string, number>;
  loadingMoreAgentIds: Set<string>;
  onLoadMoreSessions: (agent: Agent) => void;
  onShowFewerSessions: (agent: Agent) => void;
  /** The open conversation, pinned into its agent's list even when it falls outside the window. */
...
export function WorkspaceNav({
  ...
  // The page size doubles as the "Show more" step; the operator sets it in Settings → Interface.
  ...
  /** One conversation row. Split out of `renderAgent` so the date groups can each render rows. */
  const renderSession = (agent: Agent, sn: Session) => {
  ...
  const renderAgent = (agent: Agent, isSub: boolean) => {
    ...
    // The open conversation always shows, even when it sits past the loaded window — otherwise the
    // chat on the right would have no row on the left, and re-selecting it would be impossible.
    const pinned =
      activeSession &&
      activeSession.agent_id === agent._id &&
      !sessions.some((s) => s._id === activeSession._id)
        ? activeSession
        : null;
    // Re-sorted rather than simply prepended: the pinned row belongs at its own place in time.
    const shown = pinned
      ? [pinned, ...sessions].sort((a, b) => b.updated_at.localeCompare(a.updated_at))
      : sessions;
    const total = sessionTotals[agent._id] ?? sessions.length;
    // The pinned row is itself one of the unloaded ones, so it no longer counts as hidden.
    const hidden = Math.max(0, total - sessions.length - (pinned ? 1 : 0));
    const loadingMore = loadingMoreAgentIds.has(agent._id);
    return (
          ...
          <div className="ml-3 border-l border-border pl-1.5">
            ...
            {groupByDate(shown).map((group) => (
              <div key={group.label} className="mt-1 first:mt-0">
                <div className="px-2 pb-0.5 pt-1 text-[9px] font-semibold uppercase tracking-wider text-slate-600">
                  {group.label}
                </div>
                {group.items.map((sn) => renderSession(agent, sn))}
              </div>
            ))}

            {/* Pager: the window holds `sessions`, the agent has `total`. */}
            {hidden > 0 && (
              <button
              ...
              >
                ...
                <span className="ml-auto text-[10px] text-slate-600">{hidden} hidden</span>
              </button>
            )}
