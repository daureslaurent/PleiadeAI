...
import { Eye, Plus, RefreshCw, Server, SlidersHorizontal, Star, Trash2, Type } from 'lucide-react';
...
function ModesEditor({ endpoint: e, onPatch }: { endpoint: Endpoint; onPatch: (p: EndpointPatch) => void }) {
  ...
  function add(type: EndpointMode['type']) {
    const mode: EndpointMode = {
      // Minted here so the row keeps its identity across the save; the backend preserves an id it
      // is given, and a conversation references it from the moment it is selected.
